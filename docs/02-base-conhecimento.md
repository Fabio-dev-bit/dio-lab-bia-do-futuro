...
> Descreva como seu agente acessa a base de conhecimento.
 ...
 Existem duas possibilidades, injetar os dados diretamente no prompt (Crtl + C, Ctrl + V) ou carregar os arquivos via código, como no exemplo abaixo:
``` python
import pandas as pd
import json
...
PRODUTOS DISPONVEIS PARA ENSINO (data/produtos_financeiros.json):
...
O exemplo de contexto montado abaixo, se baseia nos dados originais da base de conhecimento, mas os sintetiza deixando apenas as informações mais relevantes, otimizando assim o consumo de tokens. Entretanto, vale lembrar que mais importante que econimizar tokens, é ter todas as informações relevantes disponíveis em seu contexto.
